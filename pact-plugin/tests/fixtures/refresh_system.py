"""
Refresh-system test fixtures and transcript factories.

Provides JSONL transcript factories and pytest fixtures consumed by the
refresh-system tests (test_extract_workflow_state, test_step_extractor,
test_workflow_detector, test_transcript_parser, test_checkpoint_builder).

Direct-import symbols (factories): consumed via
``from fixtures.refresh_system import ...``.

Pytest-fixture symbols (tmp_transcript, sample_checkpoint, transcript
fixtures): re-exported from tests/conftest.py to remain injectable in
any test_*.py file.
"""
