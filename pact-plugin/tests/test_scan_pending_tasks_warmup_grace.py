...
What is verified:
- When the most recent `scan_armed` event's `ts` (ISO-8601 UTC, parsed
  via strptime to epoch) is within the warmup-grace window (now - 30s),
  the bash block exits 0 and short-circuits — no further stdout.
...
# strptime conversion in Step 0 (the read returns a string the bash
...
def _write_journal(session_dir: Path, event_type: str, payload: dict) -> Path:
    ...
    the on-disk shape session_journal.py produces. The default `ts`
    uses make_event's canonical `%Y-%m-%dT%H:%M:%SZ` format; callers
    may override `ts` in `payload` to control the timestamp the
    Step 0 strptime-based extractor will consume."""
...
def test_step_0_skips_within_grace_window(tmp_path, step_0_bash_template):
    """When `scan_armed.ts = now - (WARMUP_GRACE_SECONDS - 150)s` (well
    inside the warmup-grace window), Step 0 must exit 0 without falling
    through. Discriminator: sentinel NOT in stdout.
    ...
    assert SENTINEL not in result.stdout, (
        f"Step 0 should have SHORT-CIRCUITED on scan_armed.ts=now-{elapsed} "
...
def test_step_0_falls_through_outside_grace_window(tmp_path, step_0_bash_template):
    """When `scan_armed.ts = now - (WARMUP_GRACE_SECONDS + 120)s` (well
    outside the warmup-grace window), Step 0 must fall through.
    Discriminator: sentinel IN stdout.
    ...
    assert SENTINEL in result.stdout, (
        f"Step 0 should have FALLEN THROUGH on scan_armed.ts=now-{elapsed} "
        f"(elapsed={elapsed} > WARMUP_GRACE_SECONDS={WARMUP_GRACE_SECONDS}). "
...
def test_step_0_falls_through_on_negative_delta(tmp_path, step_0_bash_template):
    """When `scan_armed.ts` is in the future (clock skew, journal
    corruption, or adversarial write), the delta `(now - armed_epoch)`
    is negative. Without the `[ $delta -ge 0 ]` guard, `negative -lt 300`
    would always be true and the gate would short-circuit indefinitely —
    ...
    future-ts fixture and the sentinel would be absent from
    stdout).
    """
    session_dir = tmp_path / "session"
    armed_epoch = int(time.time()) + 100
    _write_journal(session_dir, "scan_armed", {"ts": _iso_ts(armed_epoch)})
    ...
    assert SENTINEL in result.stdout, (
        f"Step 0 should fall through on future-dated scan_armed.ts (delta < 0). "
...
def test_step_0_uses_latest_when_multiple_scan_armed_events(tmp_path, step_0_bash_template):
    ...
    session re-armed after a stop/start cycle, leaving an old ts plus a
    recent ts), Step 0 MUST consume the MOST RECENT one.
    ...
    appending an old (out-of-grace) ts FOLLOWED BY a recent (in-grace)
    ts must yield a SKIP. If the reader ever regressed to a forward
    scan (returning the FIRST match), the elapsed value would be the
    old one and Step 0 would fall through — silently degrading the
    warmup-grace guarantee.

    Counter-test-by-revert: changing the implementation of
    `_read_last_event_at` from reverse-scan to forward-scan flips this
    test (the old ts would be used, elapsed > grace, sentinel
    appears in stdout).
    """
    session_dir = tmp_path / "session"
    # Append two scan_armed events in journal order:
    #   1. Old ts (well outside grace — would force fall-through if read).
    #   2. Recent ts (well inside grace — should force SKIP).
    ...
    now = int(time.time())
    _write_journal(session_dir, "scan_armed", {"ts": _iso_ts(now - old_elapsed)})
    _write_journal(session_dir, "scan_armed", {"ts": _iso_ts(now - recent_elapsed)})
    ...
    assert SENTINEL not in result.stdout, (
        ...
        f"present in stdout indicates the gate used the OLD ts — "
