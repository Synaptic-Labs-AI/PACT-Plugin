...
# ---------------------------------------------------------------------------
# Compaction attribution: only a teammate verdict skips the write
# ---------------------------------------------------------------------------


class TestCompactionAttributionGate:
    """postcompact_archive asks shared.compaction_owner whose compaction a
    lead-shaped frame is, journals the verdict, and skips the write only for a
    teammate. The predicate is patched on its own module, which the hook calls
    through, so the patch reaching the call is itself under test."""

    SID = "4ec31948-bbe5-4ef4-841c-631d1ef31e61"

    def _run(self, monkeypatch, capsys, verdict, **frame_overrides):
        import io
        ...
        def attribute(received, **_clocks):
            asked.append(received)
            return verdict

        monkeypatch.setattr(compaction_owner, "attribute_compaction", attribute)
        monkeypatch.setattr("sys.stdin", io.StringIO(json.dumps(frame)))
        with pytest.raises(SystemExit) as exc:
            postcompact_archive.main()
        journal = destination.parent / "session-journal.jsonl"
        events = (
        ...
        return {
            ...
            "file": destination.read_text(encoding="utf-8"),
            "verdicts": [(e["verdict"], e["basis"]) for e in events if e.get("type") == "compaction_attributed"],
            "asked": asked,
        }

    def test_a_teammate_verdict_leaves_the_lead_summary_and_records_it(self, monkeypatch, capsys):
        result = self._run(monkeypatch, capsys, ("teammate", "content"))
        assert result["code"] == 0
        assert json.loads(result["stdout"]) == {"suppressOutput": True}
        assert result["file"] == "THE LEAD'S OWN SUMMARY"
        assert result["verdicts"] == [("teammate", "content")]

    @pytest.mark.parametrize("verdict", [("lead", "content"), ("unknown", "deadline"), ("unknown", "no_signal")])
    def test_a_lead_or_unknown_verdict_writes_the_summary_as_before(self, monkeypatch, capsys, verdict):
        result = self._run(monkeypatch, capsys, verdict)
        assert result["code"] == 0
        assert json.loads(result["stdout"]) == {"suppressOutput": True}
        assert result["file"] == "THE NEW SUMMARY"
        assert result["verdicts"] == [verdict]

    @pytest.mark.parametrize("override", [{"agent_type": "pact-architect"}, {"compact_summary": ""}])
    def test_the_predicate_is_not_asked_when_no_write_would_happen(self, monkeypatch, capsys, override):
        result = self._run(monkeypatch, capsys, ("teammate", "content"), **override)
        assert result["asked"] == []
