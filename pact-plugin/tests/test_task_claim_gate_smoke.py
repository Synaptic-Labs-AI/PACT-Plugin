...
decisions so a regression surfaces fast:
  ...
  S3. Tmux + registry-confident identity + exactly one owned-unblocked-pending
      task → task-specific single nudge naming the id; M1 does NOT flip
...
# ─── S3: tmux + confident + single candidate → single nudge, NO flip (M1) ────


def test_tmux_single_candidate_nudge_no_flip(tmp_path, monkeypatch):
    _seed_config(tmp_path)
    _mock_registry(monkeypatch, f"{TEAMMATE}@{TEAM}")
    _seed_task(tmp_path, "B", subject="devops: implement", owner=TEAMMATE,
               status="pending", blockedBy=[])
    ...
    assert advisory is not None
    assert "#B" in advisory and "in_progress" in advisory
    # M1 never mutates: the task stays pending.
...
def test_unblocked_predicate_completed_vs_open_blocker(tmp_path, monkeypatch):
    ...
    # A completed → B unblocked → eligible (single nudge names #B).
    advisory = gate._evaluate(_payload(session_id=TMUX_SID, agent_type=TEAMMATE))
    assert advisory is not None and "#B" in advisory

    # Flip A back to pending → B is blocked → NO-OP.
    _seed_task(tmp_path, "A", subject="devops: TEACHBACK for x", owner=TEAMMATE,
               status="pending")
