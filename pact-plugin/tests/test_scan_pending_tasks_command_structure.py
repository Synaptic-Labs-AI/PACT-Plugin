...
def test_step_0_5_self_teardown_present_in_operation(cmd_text):
    ...
    `scan_disarmed` event types by name; must include the uniform-
    strptime parsing of each event's auto-stamped `ts` field via the
    format literal '%Y-%m-%dT%H:%M:%SZ' (matching
    session_journal.make_event under #821's ts-unification);
    ...
    assert "%Y-%m-%dT%H:%M:%SZ" in step_0_5_body, (
        ...
        "matching session_journal.make_event's strftime call — under "
        "#821's ts-unification, all three pending-scan events "
        "(scan_armed, scan_disarmed, teardown_request) carry only the "
        "auto-stamped `ts` field, and the Step 0.5 bash extractors "
        "parse all three via strptime against this literal. Drift "
        "between this literal and make_event's format string breaks "
