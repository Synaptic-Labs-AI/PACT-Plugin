...
#   - test_has_same_teammate_continuation.py: 38+ collection errors
#     (the predicate is gone — module-level import fails on
#      `from shared.wake_lifecycle import has_same_teammate_continuation`).
#   - test_wake_lifecycle_bug_a_defer_teardown.py: 2-3 fail
#     (the defer cases without the count gate fall back to Teardown emit;
#      tests asserting suppressOutput now see hookSpecificOutput) + 1 race-
#      deleted continuation test fails.
# Total cardinality on revert: ~6+ test failures plus 38+ collection errors.
...
# EXPECTED CARDINALITY ON REVERT:
#   - test_wake_lifecycle_bug_b_rearm.py::TestBugBReArmOnTeammateClaim: 5 fail
#     (re-Arm branch missing → no Arm emit on teammate claims;
#      tests asserting hookSpecificOutput see suppressOutput).
#   - test_wake_lifecycle_bug_b_rearm.py::test_sequence_teardown_then_claim_emits_rearm:
#     1 fail at Step 2 (re-Arm branch missing).
#   - test_is_pending_to_in_progress_transition.py: 8 fail/error
#     (7 predicate cells fail + 1 production-shape constraint test
#      fails or errors when the predicate symbol is gone).
# Total cardinality on revert: ~14 test failures.
```
...
# EXPECTED CARDINALITY ON REVERT: 5 fail
#   - test_marker_substring_present_in_doc_surface[*]: 4 fail (one per surface)
#   - test_marker_substring_consistent_across_all_surfaces: 1 fail
#   - test_ssot_anchor_present_in_completion_authority: 1 fail (likely)
#   - test_persona_cross_refs_to_ssot_anchor: 1 fail (likely)
# Total: ~5-7 fail per Bug C surface revert.
...
| `test_read_trigger_precondition_pinned.py` (7 tests) | All pass on fix | 5-7 fail on revert |
| Existing `test_inbox_wake_lifecycle_emitter.py` (full file) | All pass on fix; no regressions | — |
| Existing `test_inbox_wake_lifecycle_helper.py` (full file) | All pass on fix; no regressions | — |

---

## Known limitations

1. **Secretary-on-secretary Bug A scenario is NOT FIXED in this PR.** Architect
   spec cell-6 returns False on exempt-agentType continuations (the
   SELF_COMPLETE_EXEMPT_AGENT_TYPES carve-out applies to the predicate too).
   The empirical secretary Bug A in session pact-8159e827 happens because the
   secretary's session-briefing teachback chain is the FIRST teammate work, so
   the eager Teardown when secretary completes its teachback removes the
   Monitor before non-exempt teammates dispatch later. The fix for that
   scenario is Bug B (re-Arm on the next non-exempt teammate's claim). See
   the documentation-in-code test
   `test_secretary_bug_a_documented_not_fixed_in_this_pr` for the pinned
   architectural decision and the follow-up issue
   `wake_lifecycle: SELF_COMPLETE_EXEMPT_AGENT_TYPES decoupling for wake vs
   self-completion` for the proper fix.
