...
| _pending — tmux mode under non-default CLAUDE_CONFIG_DIR (e.g. `export CLAUDE_CONFIG_DIR=~/.claude-kimi` in tmux)_ | | 4.4.13 | /4 | n/a | (a) dispatch unblocks PASS/FAIL · (b) agents resolve PASS/FAIL · (c) @-ref resolves non-empty PASS/FAIL · (d) session-dir under `$CONFIG/pact-sessions` PASS/FAIL. |
