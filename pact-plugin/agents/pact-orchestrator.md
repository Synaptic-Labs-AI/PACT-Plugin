...
- **Team resolution** — read `team_name` from the session context: the hook-delivered value is authoritative; the project `CLAUDE.md` Current Session block is a fallback (used only when hook context is lost, and can be stale). The platform pre-creates exactly one session team; use it (you do not create it). Every specialist dispatch requires the team to exist.
