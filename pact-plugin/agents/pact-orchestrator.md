...
3. `TaskUpdate(A_id, owner="{name}", addBlocks=[B_id], metadata={"lead_session_id": pact_session_context["session_id"]})` — assign Task A to the teammate, wire it as the gate that unblocks Task B, AND stamp `metadata.lead_session_id` with the lead's current `session_id`. The field is set-at-creation and immutable thereafter; it is consumed by [`/PACT:scan-pending-tasks`](../commands/scan-pending-tasks.md#same-session-identity-gate) as the Layer 3 cross-session-contamination defense.
4. `TaskUpdate(B_id, owner="{name}", addBlockedBy=[A_id], metadata={"lead_session_id": pact_session_context["session_id"]})` — assign Task B to the same teammate, explicitly mirror the block edge, AND stamp `metadata.lead_session_id`. Do NOT pre-set `status="in_progress"` on either task — the teammate self-claims on arrival. Do NOT modify `lead_session_id` after initial set — it is provenance, not state.
