...
2. **Cron-fire is an alarm-clock, not a content marker.** When the `/PACT:scan-pending-tasks` cron fires, that ends your idle period at the next between-tool-call boundary so the platform can deliver any queued SendMessages alongside the scan turn — but the cron prompt itself contains no teammate content. See [scan-pending-tasks §Cron-Fire Origin](../commands/scan-pending-tasks.md#cron-fire-origin) for the canonical harness-origin classification. Do NOT skip ahead to a raw JSON read on cron-fire; wait for the wake-signal SendMessage payload to surface in your context (or for the scan body's G3 raw-read step to discover the artifact on disk).
3. **Raw read MUST follow SendMessage receipt, not precede it.** The ordering is: teammate writes `metadata.teachback_submit` → teammate sends notify SendMessage → platform poller delivers between-tool-call (or the cron-fired scan body opens an idle boundary) → your turn opens with the SendMessage in context → THEN you read `cat ~/.claude/tasks/{team_name}/{A_id}.json | jq .metadata.teachback_submit`. Reversing this order produces false-empty reads that have historically triggered false-positive rejection cycles; the [scan-pending-tasks §Guardrails G4 race-window-skip](../commands/scan-pending-tasks.md#guardrails) is the per-fire structural enforcement of this rule.
