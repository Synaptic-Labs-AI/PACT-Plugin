...
The platform's `useInboxPoller` only delivers queued `SendMessage` between tool calls; long-running tool calls leave inbound completion-authority signals stuck until the next idle boundary. The lead-side scan mechanism (lead-only — teammates rely on the standard idle-delivery channel) closes this gap with a recurring cron entry that fires `/PACT:scan-pending-tasks` every 3 minutes between tool calls, opening an idle boundary where the platform's poller delivers queued messages and the scan reads completion-authority artifacts directly from disk. The first-fire false-positive window is bounded by a 180-second warmup-grace skip in the scan body (Step 0); the warmup-grace constant is coupled in lockstep to the cron interval — see [start-pending-scan.md §CronCreate Block](../commands/start-pending-scan.md#croncreate-block) for the coupling invariant.
