...
These triggers are idempotent — safe to fire even if HANDOFFs were already processed. The secretary discovers completed tasks via the breadcrumb file (primary source) and cross-references with `TaskList` (supplementary). `TaskList` may be incomplete in long sessions due to platform garbage collection of older task files.
