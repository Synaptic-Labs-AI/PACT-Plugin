...
**Mechanism**: SendMessage each specialist who contributed:
...
- **Correction provided**: Update synthesis, re-verify with the correcting specialist only
- **Specialist unavailable** (shut down): Treat synthesis as accepted for that perspective, note in plan's Limitations section

This verifies the orchestrator's understanding matches what specialists actually recommended — preventing the plan from misrepresenting specialist input. Same agreement verification mechanism as orchestrate phase boundaries. See [pact-ct-teachback.md](../protocols/pact-ct-teachback.md).
