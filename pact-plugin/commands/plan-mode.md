...
1. `TaskCreate(subject="{specialist}: plan consultation for {feature}", description="PLANNING CONSULTATION ONLY — No implementation.\n\nTask: {task description}\n\n[full template content from above]")`
   - Add to description: "Send a teachback to lead restating your understanding of what you're being asked to consult on before providing your analysis. If upstream context is referenced (e.g., prior preparation output, an existing plan being revised), read it via TaskGet first. See agent-teams skill for teachback format."
