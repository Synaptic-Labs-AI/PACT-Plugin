...
- `prompt="/PACT:scan-pending-tasks"` — BYTE-IDENTICAL to the prompt in [scan-pending-tasks.md](scan-pending-tasks.md) frontmatter and to the suffix filter in [stop-pending-scan.md](stop-pending-scan.md). INV-1 cross-skill identifier. Silent drift breaks the CronList lookup for both idempotency (here) and teardown (in stop-pending-scan), causing orphan-cron accumulation and silent re-arm failure. Verified by structural test asserting byte-identity across the 3 files.
