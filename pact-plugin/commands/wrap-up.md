...
Invoke `Skill("PACT:stop-pending-scan")` before worktree cleanup. This deletes the lead's `/PACT:scan-pending-tasks` cron entry; it is the hook-silent-fail safety-net path that runs alongside the PostToolUse 1→0 last-active Teardown directive. Idempotent and best-effort — see [stop-pending-scan §Operation](stop-pending-scan.md#operation).
