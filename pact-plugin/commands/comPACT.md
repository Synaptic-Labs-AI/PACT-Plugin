...
- [ ] **HANDOFF curation**: Send completed agent task IDs to `pact-memory-agent` via `SendMessage`:
  ```
  SendMessage(to="memory-agent",
    message="[lead→memory-agent] Curate HANDOFFs for comPACT completion. Task IDs: #X. Read via TaskGet for HANDOFF metadata. Save institutional knowledge to pact-memory. Report summary when done.",
    summary="Curate HANDOFF: task #X")
  ```
