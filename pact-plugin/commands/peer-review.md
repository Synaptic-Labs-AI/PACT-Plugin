...
5. **Calibration save**: Create memory save tasks for reviewers who found significant patterns. When work stabilizes, unblock save tasks so idle agents can load `Skill("pact-memory")` and save review calibration data with entity `review_calibration`. This runs unconditionally — even clean reviews provide signal. Skip only for trivial single-file PRs.
