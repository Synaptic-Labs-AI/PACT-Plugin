...
When force-termination drops the team's active-task count to zero, invoke `Skill("PACT:inbox-wake")` and execute the Teardown operation. This stops the lead's Monitor task and unlinks the STATE_FILE as the parallel-safety-net path alongside the PostToolUse 1→0 last-active Teardown directive. Skip if other active teammate tasks remain (Monitor stays armed for them). Idempotent and best-effort — see [skills/inbox-wake/SKILL.md §Teardown Block](../skills/inbox-wake/SKILL.md#teardown-block).
