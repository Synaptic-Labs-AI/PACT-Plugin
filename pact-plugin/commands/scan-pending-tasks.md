...
**Audit**: Emit-Nothing-If-Empty prevents the empty-scan-narration failure mode. If empty scans emitted "Nothing pending" (or any prose), 12 fires per hour of empty scans during active teammate work would fill the transcript with 12 useless lines per hour. An editing LLM tempted to "emit a one-liner confirming the scan ran" misunderstands the design: the cron's existence in `CronList` IS the confirmation that the scan is running; per-fire output is the failure mode. The combined effect of No-Narration + Emit-Nothing-If-Empty is: the user sees output from the scan ONLY when the scan actually accepts a teammate's artifact. All other cron fires are silent.
