...
3. **Your CLAUDE.md migration is mostly automatic** — one manual cleanup step for direct v3.x upgraders. Specifically:
  - **Project CLAUDE.md `PACT_ROUTING` block — manual removal for direct v3.x → v4.2.15+ upgraders**: v4.0.x and v4.1.x shipped an orphan-stripper that auto-removed the stale `<!-- PACT_ROUTING_START ... --> ... <!-- PACT_ROUTING_END -->` block from your project CLAUDE.md on each session start. The stripper retired in v4.2.15. If you ran any v4.0.x or v4.1.x session, the block is already gone. If you upgrade directly from v3.x to v4.2.15+ without an interim session, delete the `<!-- PACT_ROUTING_START ... -->` ... `<!-- PACT_ROUTING_END -->` block from your project CLAUDE.md manually before first session start.
