...
| L1-SG-03 | ALERT blocks phase scope, HALT blocks feature scope; signals persist to resolution | `protocols/algedonic.md` task-system integration: ALERT blocks the phase task, HALT blocks the feature task via dependency edges; signal tasks persist and unblock on Principal resolution | satisfied | | The SHOULD-strength acceptor-side clause (resolution accepted only from a Principal Channel event) is not realized: resolution acceptance is instruction-enforced, with no substrate-distinguished channel to bind to |
| L1-SG-04 | HALT resumes only on Principal acknowledgment; override recorded on the signal record, non-carrying | `protocols/algedonic.md` resolution options: explicit acknowledgment required; override documented with risk-specific justification; a materialized overridden risk re-signals | satisfied-with-deviation | | The override recording locus is missed: the justification is instructed to session notes and the decision log, not onto the signal record's metadata or a Journal event; signal-task persistence, the re-signal rule, and no-carry-forward are realized as keyed. Separately, the SHOULD-strength acceptor-side clause (acknowledgment as a Principal Channel event; free-form content not treated as acknowledgment) is not realized as-built |
...
| L2-PG-02 | Nesting depth at most one; nested cycles report to the parent via HANDOFF | `protocols/pact-s1-autonomy.md` nesting limit (1 level maximum); `commands/rePACT.md` (maximum nesting 1 level; results reported to the parent via HANDOFF); nested-cycle signal bypass per `protocols/algedonic.md`; the depth-1 cap is pinned literally at both loci, matching the keyed cap | satisfied | | |
| L2-VS-01 | Four dimensions, integers 1 to 4; total recorded in the work record's variety stamp | `hooks/shared/variety_scorer.py` (per-dimension and total bounds enforced); the total lands in the per-dispatch variety stamp (`protocols/pact-variety.md`) | satisfied | | |
...
| L2-PS-02 | Plan-justified skips preceded by a declared-signal check; outcome recorded in the skip annotation | `protocols/pact-completeness.md` seven-signal set with mechanical detection guidance, consumed as a layer of the skip gate in `commands/orchestrate.md`; the pass outcome is recorded as the skip annotation's reason value | satisfied | | |
| L2-AU-01 | Observation engagement declared; skips justified in a durable TaskStore or Journal record | `protocols/pact-audit.md` engagement conditions (variety at or above 7, three or more parallel coders, security-sensitive code, drift history) with a required written skip justification | satisfied-with-deviation | | The justification locus is missed: the skip justification is instructed as a coordinator output line, not a TaskStore or Journal record; the engagement conditions and mandatory-dispatch rule are declared as keyed |
| L2-AU-02 | Verdict recorded on the observer's own work record; absence means not-yet-written, never no-findings | `protocols/pact-audit.md`: the verdict is recorded on the observer's own work record and durably mirrored; non-blocking rule — an absent verdict means not written yet, never no findings | satisfied-with-deviation | | The recording clause is realized on the observer's record; the absence-interpretation clause remains instruction-only — no substrate observable distinguishes a consumer that inferred a clean observation from absence |
...
| L3-IR-01 | Irreversible integration actions preceded by a Principal Channel confirmation event; evidence recording recommended | `protocols/pact-s5-policy.md` irreversible-actions rule: structured multiple-choice confirmation prompt answerable only by the Principal; `hooks/merge_guard_post.py` mints a bounded-use authorization token on the observed approval event; `hooks/merge_guard_pre.py` denies integration commands lacking an exactly matching token | satisfied-with-deviation | | The as-built channel is hook-observed convention rather than substrate-distinguished: the guard's allow decision consumes a token file a protocol actor could originate directly (no native write restriction), so the substrate cannot guarantee a Principal confirmation event preceded the action; the SHOULD locus is also unmet — confirmation evidence lands in the token record (minted, atomically retired on consumption), not in the TaskStore or Journal |
...
| L3-QG-01 | No integration through the EnforcementPoint gate while verification is known-failing | `protocols/pact-s5-policy.md` Quality class; the change-integration interception exists (merge-guard hooks); review workflow requires the full suite green before the change-integration request; Principal merge checkpoint | satisfied-with-deviation | | The EnforcementPoint interception on change-integration commands exists, but its deny logic checks Principal authorization, not verification status: a known-failing merge is prevented by workflow discipline and review-recorded verification evidence, not by the gate itself |
| L3-SC-01 | Write boundaries or sequencing recorded in a durable substrate interface before covered work begins | `protocols/pact-s2-coordination.md` pre-parallel conflict check with boundary or sequencing assignment, persisted as boundary metadata on the phase work record and a seeded-state Journal event; scope-contract shared-file constraints (`protocols/pact-scope-contract.md`) | satisfied | | |
| L3-SC-02 | Scope contracts recorded durably before sub-scope work; completion HANDOFF reports fulfillment | `protocols/pact-scope-contract.md`: generated contracts with identity, deliverables, interfaces, and constraints including sibling-owned shared files, persisted to the scope work record's metadata at decomposition; the sub-scope HANDOFF carries a contract-fulfillment section | satisfied | | |
...
| L3-OB-01 | Overwritten verdicts preserved within the same record; overwrite visibly recorded | `hooks/task_lifecycle_gate.py`: an overwrite of an observer-authored verdict preserves the authored value to a mirror key on the same work record's metadata, routes the overwriting value aside on that record, and emits an escalating overwrite advisory (`protocols/pact-audit.md`) | satisfied | | |
