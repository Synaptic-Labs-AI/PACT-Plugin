...
**What happens**: After the agent completes, the orchestrator sends completed task IDs to `pact-memory-agent` for HANDOFF curation. The memory agent reads HANDOFF metadata, extracts institutional knowledge, and saves structured memories to pact-memory.

**Expected outcome**: The memory agent curates the HANDOFF into a structured memory object in SQLite. The memory includes context, goal, lessons_learned, decisions, and entities fields.
...
| pact-memory save fails silently | No new entries in SQLite after task | Check memory agent HANDOFF curation completed; verify SQLite extensions installed |
